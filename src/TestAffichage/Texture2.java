...
public class Texture2
{
    ...
    private Texture2(int id, int width, int height)
    {
        this.id = id;
        this.width = width;
        this.height = height;
    ...
    public static Texture2 loadTexture(String name)
    {
        ...
        try
        {
        	bimg = ImageIO.read(Texture2.class.getClassLoader().getResourceAsStream( name));
        }
        catch (IOException e)
        {
            e.printStackTrace();
        ...
        return new Texture2(textureID, bimg.getWidth(), bimg.getHeight());
    }
